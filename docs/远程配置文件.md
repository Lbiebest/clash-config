...
| enable_rule_generator      | 启用规则生成器       | 默认开启即可                           |
| overwrite_original_rules   | 覆盖原有规则         | 默认开启即可                           |
| clash_rule_base            | Clash 基础配置       | [Clash 文件的配置内容]                   |

远程配置 `ini` 参考
[SSR去广告规则/GFWList规则/Clash规则碎片](https://github.com/ACL4SSR/ACL4SSR/tree/master)

[Clash 文件的配置内容]:./配置文件.md
